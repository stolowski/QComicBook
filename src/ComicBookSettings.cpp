...
const EnumMap<Size> ComicBookSettings::size2string[] = {
    {"original", Original},   {"fitwidth", FitWidth}, {"fitheight", FitHeight},
    {"wholepage", WholePage}, {"bestfit", BestFit},   {QString{}}};
...
ComicBookSettings::ComicBookSettings()
    : QObject(), m_bkpath(QString{}), m_thpath(QString{}), m_dirsok(false) {
  m_cfg = new QSettings();
  //	cfg->insertSearchPath(QSettings::Unix, QDir::homeDirPath() +
  //"/.qcomicbook");
...
bool ComicBookSettings::checkDirs() {
  ...
  m_thpath = QStandardPaths::writableLocation(QStandardPaths::CacheLocation) +
...
const QString &ComicBookSettings::bookmarksDir() { return m_bkpath; }

const QString &ComicBookSettings::thumbnailsDir() { return m_thpath; }
...
void ComicBookSettings::load() {
  ...
  m_pagesize = convert(
      size2string, m_cfg->value(OPT_PAGESIZE, size2string[0].str).toString());
  ...
  m_viewtype =
      convert(viewtype2string,
              m_cfg->value(OPT_VIEWTYPE, viewtype2string[0].str).toString());
  m_cfg->endGroup();
...
bool ComicBookSettings::embedPageNumbers() const { return m_embedpagenumbers; }

bool ComicBookSettings::smallCursor() const { return m_smallcursor; }

bool ComicBookSettings::twoPagesMode() const { return m_twopages; }

bool ComicBookSettings::japaneseMode() const { return m_japanese; }

bool ComicBookSettings::continuousScrolling() const { return m_contscroll; }

ViewType ComicBookSettings::viewType() const { return m_viewtype; }

bool ComicBookSettings::scrollbarsVisible() const { return m_scrollbars; }
...
Size ComicBookSettings::pageSize() const { return m_pagesize; }

bool ComicBookSettings::smoothScaling() const { return m_smoothscaling; }

QString ComicBookSettings::lastDir() const { return m_lastdir; }

const QStringList &ComicBookSettings::recentlyOpened() const {
  return m_recent;
}

QColor ComicBookSettings::background() const { return m_bgcolor; }

int ComicBookSettings::cacheSize() const { return m_cachesize; }

bool ComicBookSettings::cacheAutoAdjust() const { return m_cacheadjust; }

bool ComicBookSettings::cacheThumbnails() const { return m_cachethumbs; }

int ComicBookSettings::thumbnailsAge() const { return m_thumbsage; }

bool ComicBookSettings::preloadPages() const { return m_preload; }

bool ComicBookSettings::confirmExit() const { return m_confirmexit; }

bool ComicBookSettings::autoInfo() const { return m_autoinfo; }

bool ComicBookSettings::fullScreenHideMenu() const { return m_fscrhidemenu; }
...
bool ComicBookSettings::showStatusbar() const { return m_statusbar; }

const QFont &ComicBookSettings::infoFont() const { return m_font; }

bool ComicBookSettings::showSplash() const { return m_showsplash; }
...
QString ComicBookSettings::tmpDir() const { return m_tmpdir; }

bool ComicBookSettings::showDonationDialog() const { return m_donationdlg; }
...
void ComicBookSettings::viewType(ViewType t) {
  if (t != m_viewtype) {
    m_cfg->setValue(GRP_VIEW OPT_VIEWTYPE,
                    convert(viewtype2string, m_viewtype = t));
  }
}
...
void ComicBookSettings::pageSize(Size s) {
  if (s != m_pagesize) {
    m_cfg->setValue(GRP_VIEW OPT_PAGESIZE,
                    convert(size2string, m_pagesize = s));
  }
}
