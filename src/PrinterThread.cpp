...
PrinterThread::PrinterThread(QSharedPointer<ImgSink> sink,
                             QSharedPointer<QPrinter> printer,
                             QAbstractPrintDialog::PrintRange range, int from,
                             int to)
    : QThread(), m_sink(sink), m_printer(printer), m_range(range), m_from(from),
      m_to(to), m_abort(false) {
...
PrinterThread::~PrinterThread() { m_printer.clear(); }
