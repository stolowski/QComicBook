...
SimplePageView::SimplePageView(QWidget *parent, int physicalPages,
                               const ViewProperties &props)
    : PageViewBase(parent, physicalPages, props), wheelupcnt(0),
      wheeldowncnt(0), m_currentPage(0) //??
      ,
      imgLabel(NULL) {
  _DEBUG;
...
SimplePageView::~SimplePageView() { _DEBUG; }
...
void SimplePageView::propsChanged() {
  _DEBUG;
  if (imgLabel) {
    if ((props.twoPagesMode() && !imgLabel->hasTwoPages()) ||
        (imgLabel->hasTwoPages() && !props.twoPagesMode())) {
      recreateComicPageImage();
    }
...
void SimplePageView::gotoPage(int n) {
  ...
  if (n >= 0 && n < numOfPages()) {
    ...
    if (cfg.preloadPages()) {
      n = nextPage(n);
      _DEBUG << "preloading" << n;
      addRequest(n, props.twoPagesMode() &&
                        n < numOfPages()); // request two pages if not last page
    }
  }
}
...
void SimplePageView::wheelEvent(QWheelEvent *e) {
  if (imgLabel) {
    if (e->angleDelta().y() > 0) // scrolling up
    {
      if (imgLabel->height() <= height() ||
          (onTop() && ++wheelupcnt > EXTRA_WHEEL_SPIN)) {
        e->accept();
        wheelupcnt = 0;
    ...
    {
      if (imgLabel->height() <= height() ||
          (onBottom() && ++wheeldowncnt > EXTRA_WHEEL_SPIN)) {
        e->accept();
        wheeldowncnt = 0;
