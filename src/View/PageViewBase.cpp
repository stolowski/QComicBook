...
PageViewBase::PageViewBase(QWidget *parent, int physicalPages,
                           const ViewProperties &props)
    : QGraphicsView(parent), m_physicalPages(physicalPages), props(props),
      smallcursor(0), lens(0) {
  ...
  connect(ImageTransformThread::get(),
          SIGNAL(jobCompleted(const ImageJobResult &)), this,
          SLOT(jobCompleted(const ImageJobResult &)));
}
...
void PageViewBase::mouseMoveEvent(QMouseEvent *e) {
  if (e->buttons()) {
    if (last.x() >= 0) {
      auto const delta = last - e->position();
    ...
    if (lens) {
      lens->setPos(e->pos() + QPointF(horizontalScrollBar()->value(),
...
void PageViewBase::scrollRight() { scrollByDelta(spdx, 0); }

void PageViewBase::scrollLeft() { scrollByDelta(-spdx, 0); }

void PageViewBase::scrollRightFast() { scrollByDelta(3 * spdx, 0); }

void PageViewBase::scrollLeftFast() { scrollByDelta(-3 * spdx, 0); }
...
void PageViewBase::setNumOfPages(int n) { m_physicalPages = n; }

int PageViewBase::numOfPages() const { return m_physicalPages; }

void PageViewBase::setRotation(Rotation r) { props.setAngle(r); }

void PageViewBase::rotateRight() { setRotation(QComicBook::Right); }

void PageViewBase::rotateLeft() { setRotation(QComicBook::Left); }

void PageViewBase::resetRotation() { setRotation(None); }

void PageViewBase::setSize(Size s) { props.setSize(s); }
...
void PageViewBase::setTwoPagesMode(bool f) { props.setTwoPagesMode(f); }

void PageViewBase::setMangaMode(bool f) { props.setMangaMode(f); }
...
void PageViewBase::setSmallCursor(bool f) {
  if (f) {
    ...
    const QBitmap bmp =
        QBitmap::fromData(QSize(32, 32), bmp_bits, QImage::Format_Mono);
    const QBitmap msk =
        QBitmap::fromData(QSize(32, 32), msk_bits, QImage::Format_Mono);
...
void PageViewBase::showPageNumbers(bool f) { props.setPageNumbers(f); }

void PageViewBase::enableScrollbars(bool f) {
  const Qt::ScrollBarPolicy s =
      f ? Qt::ScrollBarAsNeeded : Qt::ScrollBarAlwaysOff;
  setVerticalScrollBarPolicy(s);
  setHorizontalScrollBarPolicy(s);
}

QMenu *PageViewBase::contextMenu() const { return context_menu; }

ViewProperties &PageViewBase::properties() { return props; }
...
void PageViewBase::delRequests() { m_requestedPages.clear(); }
...
void PageViewBase::updateSceneRect() {
  ...
  // this is the same QGraphicsScene::itemsBoundingRect(), except for it skips
  // lens.
