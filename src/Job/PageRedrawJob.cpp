...
void PageRedrawJob::execute() {
  ...
  if (m_image[1]) // 2 pages mode
  {
    ...
    for (int i = 0; i < 2; i++) {
      const int j(i ^ swap);
      if (m_image[j]->height() <
          std::max(m_image[0]->height(), m_image[1]->height())) {
        p.fillRect(
            i * m_image[j]->width(), m_image[j]->height(), m_image[j]->width(),
            m_sourceSize.height() - m_image[j]->height(), m_props.background);
    ...
    if (m_props.pageNumbers) {
      ...
      drawPageNumber(std::max(m_numbers[swap], m_numbers[1 ^ swap]), p, m_width,
                     m_height);
    }
  } else // 1 page mode
  {
...
void PageRedrawJob::drawPageNumber(int page, QPainter &p, int x, int y) {
  ...
  p.setPen(Qt::black);
  p.fillRect(x - txtw - 5, y - 2 - mtr.height(), txtw + 5, mtr.height() + 4,
             Qt::white);
  p.drawText(x - txtw - 4, y - 4, pagestr);
}

void PageRedrawJob::setSourceSize(const QSize &size) { m_sourceSize = size; }
