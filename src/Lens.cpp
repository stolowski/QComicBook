...
Lens::Lens(const QSize &size, const QColor &background, double ratio, int delay)
    : QGraphicsItem(), m_pixmap(0), m_time(0), m_delay(delay), m_size(size),
      m_background(background), m_ratio(ratio) {
  setZValue(1000.0f);
...
void Lens::paint(QPainter *painter, const QStyleOptionGraphicsItem *opt,
                 QWidget *widget) {
  _DEBUG;
  if (m_pixmap) {
    painter->drawPixmap(opt->exposedRect, *m_pixmap,
                        QRectF(0, 0, m_pixmap->width(), m_pixmap->height()));
    painter->setPen(QPen(Qt::black, 1.0f));
    painter->drawRect(-m_size.width() / 2, -m_size.height() / 2, m_size.width(),
                      m_size.height());
  }
}

QRectF Lens::boundingRect() const {
  return QRectF(-m_size.width() / 2, -m_size.height() / 2, m_size.width(),
                m_size.height());
}
...
QVariant Lens::itemChange(GraphicsItemChange change, const QVariant &value) {
  ...
  if (scene() && (change == ItemVisibleChange ||
                  (change == ItemPositionChange &&
                   (m_time == NULL || m_time->elapsed() > m_delay)))) {
    QPointF newPos =
