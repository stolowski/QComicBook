...
StatusBar::StatusBar(QWidget *parent, bool elms): KStatusBar(parent), pbar(NULL), page(NULL), imginfo(NULL), name(NULL)
{
	if (elms)
	{
		createInfoElements();
...
void StatusBar::setProgress(int n, int total)
{
	...
	else
	{
		if (!pbar)
		{
			...
			pbar->setFixedHeight(12); //this is a bit ugly... gives chance the sizebar's height is not increased
			addWidget(pbar, 1);
			pbar->show();
		}
