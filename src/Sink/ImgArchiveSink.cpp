...
ImgArchiveSink::ImgArchiveSink() : ImgDirSink() { init(); }
...
ImgArchiveSink::~ImgArchiveSink() { ImgArchiveSink::close(); }
...
bool ImgArchiveSink::fileHandler(const QFileInfo &finfo) {
  ...
  if (finfo.isFile()) {
    ...
    ArchiversConfiguration::instance().getExtractArguments(
        fullname, extractargs, listargs);
    if (extractargs.size()) {
...
void ImgArchiveSink::init() {
  ...
  connect(pinf, SIGNAL(readyReadStandardOutput()), this,
          SLOT(infoStdoutReady()));
  connect(pinf, SIGNAL(finished(int, QProcess::ExitStatus)), this,
          SLOT(infoExited(int, QProcess::ExitStatus)));
  connect(pext, SIGNAL(readyReadStandardOutput()), this,
          SLOT(extractStdoutReady()));
  connect(pext, SIGNAL(finished(int, QProcess::ExitStatus)), this,
          SLOT(extractExited(int, QProcess::ExitStatus)));
}
...
int ImgArchiveSink::extract(const QString &filename, const QString &destdir,
                            QStringList extargs, QStringList infargs) {
  if (extargs.size() == 0)
...
int ImgArchiveSink::open(
    const QString &path) // TODO: cleanup if already opened?
{
  filesnum = 0;
  archfiles.clear();
  ...
  if (info.isFile()) {
    if (info.isReadable()) {
      ...
      QStringList extractargs, listargs;
      ArchiversConfiguration::instance().getExtractArguments(path, extractargs,
                                                             listargs);
...
bool ImgArchiveSink::supportsNext() const { return true; }
...
QString ImgArchiveSink::getNext() const {
  ...
  QStringList files = dir.entryList(
      ArchiversConfiguration::instance().supportedOpenExtensions(),
