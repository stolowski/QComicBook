...
ImgDirSink::ImgDirSink(bool dirs, int cacheSize)
    : ImgSink(cacheSize), dirpath(QString{}),
      DirReader(QDir::DirsLast | QDir::Name | QDir::IgnoreCase, 6) {}

ImgDirSink::ImgDirSink(const QString &path, bool dirs, int cacheSize)
    : ImgSink(cacheSize), dirpath(QString{}),
      DirReader(QDir::DirsLast | QDir::Name | QDir::IgnoreCase, 6) {
  open(path);
}

ImgDirSink::ImgDirSink(const ImgDirSink &sink, int cacheSize)
    : ImgSink(cacheSize),
      DirReader(QDir::DirsLast | QDir::Name | QDir::IgnoreCase, 6) {
...
bool ImgDirSink::fileHandler(const QFileInfo &finfo) {
  ...
  if (knownImageExtension(fname)) {
    ...
    timestamps.insert(finfo.absoluteFilePath(),
                      FileStatus(finfo.lastModified()));
    return true;
  }
...
QStringList ImgDirSink::getDescription() const {
  ...
  {
    for (QStringList::const_iterator it = txtfiles.begin();
         it != txtfiles.end(); it++) {
      QFileInfo finfo(*it);
      QFile f(*it);
...
QStringList ImgDirSink::getAlldirs() const { return dirs; }
...
bool ImgDirSink::hasModifiedFiles() const {
  ...
  for (QMap<QString, FileStatus>::ConstIterator it = timestamps.begin();
...
bool ImgDirSink::supportsNext() const { return false; }

QString ImgDirSink::getNext() const { return QString{}; }

QString ImgDirSink::getPrevious() const { return QString{}; }
...
void ImgDirSink::removeThumbnails(int days) {
  ...
  QDir dir(ComicBookSettings::instance().thumbnailsDir(), "*.jpg",
           QDir::Unsorted, QDir::Files | QDir::NoSymLinks);
