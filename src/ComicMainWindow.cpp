...
ComicMainWindow::ComicMainWindow(QWidget *parent)
    : QMainWindow(parent), currpage(0)
#ifdef DEBUG
      ,
...
#endif
{
  ...
  connect(thumbswin, SIGNAL(requestedPage(int, bool)), this,
          SLOT(jumpToPage(int, bool)));
  ...
  connect(scaleActions, SIGNAL(triggered(QAction *)), this,
          SLOT(setPageSize(QAction *)));
  ...
  connect(viewTypeActions, SIGNAL(triggered(QAction *)), this,
          SLOT(changeViewType(QAction *)));
  ...
  connect(actionOpenArchive, SIGNAL(triggered(bool)), this,
          SLOT(browseArchive()));
  connect(actionOpenDirectory, SIGNAL(triggered(bool)), this,
          SLOT(browseDirectory()));
  connect(actionOpenNext, SIGNAL(triggered(bool)), this, SLOT(openNext()));
  connect(actionOpenPrevious, SIGNAL(triggered(bool)), this,
          SLOT(openPrevious()));
  ...
  connect(actionExitFullScreen, SIGNAL(triggered(bool)), this,
          SLOT(exitFullscreen()));
  connect(actionNextPage, SIGNAL(triggered(bool)), this, SLOT(nextPage()));
  connect(actionForwardPage, SIGNAL(triggered(bool)), this,
          SLOT(forwardPages()));
  ...
  connect(actionBackwardPage, SIGNAL(triggered(bool)), this,
          SLOT(backwardPages()));
  connect(actionQuit, SIGNAL(triggered(bool)), this, SLOT(close()));
  connect(actionFullscreen, SIGNAL(triggered(bool)), this,
          SLOT(toggleFullScreen()));
  connect(actionPreviousPage, SIGNAL(triggered(bool)), this, SLOT(prevPage()));
  connect(actionMangaMode, SIGNAL(toggled(bool)), this,
          SLOT(toggleJapaneseMode(bool)));
  connect(actionTwoPages, SIGNAL(toggled(bool)), this,
          SLOT(toggleTwoPages(bool)));
  connect(actionNextFrame, SIGNAL(triggered(bool)), this, SLOT(nextFrame()));
  connect(actionPreviousFrame, SIGNAL(triggered(bool)), this,
          SLOT(prevFrame()));
  ...
  connect(actionToggleStatusbar, SIGNAL(toggled(bool)), statusbar,
  ...
  connect(zoomgrp, SIGNAL(triggered(QAction *)), this,
          SLOT(setLensZoom(QAction *)));
  ...
  connect(menuRecentFiles, SIGNAL(selected(const QString &)), this,
          SLOT(recentSelected(const QString &)));
  connect(menuRecentFiles, SIGNAL(cleanupRequest()), menuRecentFiles,
          SLOT(removeAll()));
  ...
  connect(actionContinuousView, SIGNAL(toggled(bool)), this,
          SLOT(toggleContinousView(bool)));
  ...
  connect(actionRemoveBookmark, SIGNAL(triggered()), this,
          SLOT(removeBookmark()));
  connect(menuBookmarks, SIGNAL(triggered(QAction *)), this,
          SLOT(bookmarkSelected(QAction *)));
  connect(actionManageBookmarks, SIGNAL(triggered()), this,
          SLOT(openBookmarksManager()));
  bookmarks->load();

  //
  // Settings menu
  connect(actionToggleScrollbars, SIGNAL(toggled(bool)), this,
          SLOT(toggleScrollbars(bool)));
  ...
  connect(actionAboutDonating, SIGNAL(triggered()), this,
          SLOT(showAboutDonating()));

  //
  // Debug menu
  QComicBookDebug(QMenu *debugMenu = new QMenu(tr("Debug"), this);
                  QAction *actionDebugContinuousView =
                      new QAction(tr("Continuous View"), this);

                  connect(actionDebugContinuousView, SIGNAL(triggered()),
                          debugController, SLOT(showDebugContinuousView()));
                  ...
                  connect(actionDebugMemory, SIGNAL(triggered()),
                          debugController, SLOT(showDebugMemory()));
                  ...
                  QAction *actionDebugDisableRefresh =
                      new QAction(tr("Disable auto-refresh"), this);
                  actionDebugDisableRefresh->setCheckable(true);
                  actionDebugDisableRefresh->setChecked(true);
                  connect(actionDebugDisableRefresh, SIGNAL(triggered()),
                          debugController, SLOT(disableAutoRefresh()));
                  debugMenu->addAction(actionDebugDisableRefresh);
                  debugRefreshGroup->addAction(actionDebugDisableRefresh);

                  QAction *actionDebugRefresh1s =
                      new QAction(tr("Refresh every 1s."), this);
                  actionDebugRefresh1s->setCheckable(true);
                  connect(actionDebugRefresh1s, SIGNAL(triggered()),
                          debugController, SLOT(autoRefreshEvery1sec()));
                  debugMenu->addAction(actionDebugRefresh1s);
                  debugRefreshGroup->addAction(actionDebugRefresh1s);

                  QAction *actionDebugRefresh5s =
                      new QAction(tr("Refresh every 5s."), this);
                  actionDebugRefresh5s->setCheckable(true);
                  connect(actionDebugRefresh5s, SIGNAL(triggered()),
                          debugController, SLOT(autoRefreshEvery5sec()));
                  debugMenu->addAction(actionDebugRefresh5s);
                  debugRefreshGroup->addAction(actionDebugRefresh5s);

                  QAction *actionDebugRefresh30s =
                      new QAction(tr("Refresh every 30s."), this);
                  actionDebugRefresh30s->setCheckable(true);
                  connect(actionDebugRefresh30s, SIGNAL(triggered()),
                          debugController, SLOT(autoRefreshEvery30sec()));
  ...
  switch (cfg->pageSize()) {
  case FitWidth:
    which = actionFitWidth;
    break;
  case FitHeight:
    which = actionFitHeight;
    break;
  case BestFit:
    which = actionBestFit;
    break;
  case WholePage:
    which = actionWholePage;
    break;
  case Original:
    which = actionOriginalSize;
    break;
  }
  ...
  switch (cfg->viewType()) {
  case Simple:
    which = actionSimpleView;
    break;
  case Continuous:
    which = actionContinuousView;
    break;
  case Frame:
    which = actionFrameView;
    break;
  }
  which->setChecked(true);

  //
  // copy all menu actions; this is needed for fullscreen mode if menubar is
  // hidden
  ...
  // connect(cfg, SIGNAL(displaySettingsChanged(const QString &)), this,
  // SLOT(reconfigureDisplay())); ??
  ...
  connect(thumbnailLoader, SIGNAL(thumbnailLoaded(const Thumbnail &)),
          thumbswin, SLOT(setThumbnail(const Thumbnail &)));
...
void ComicMainWindow::setupComicImageView() {
  ...
  case Continuous:
    ...
    QComicBookDebug(debugController->setView(
        dynamic_cast<ContinuousPageView *>(view.data())););
    frameDetect->clear();
    break;
  case Simple:
  ...
  case Frame: {
    ...
    connect(pageLoader, SIGNAL(pageLoaded(const Page &)), frameDetect,
            SLOT(process(const Page &)));
    connect(frameDetect, SIGNAL(framesReady(const ComicFrameList &)), frameView,
            SLOT(setFrames(const ComicFrameList &)));
  } break;
  default:
    break;
  }
  ...
  connect(actionPageBottom, SIGNAL(triggered(bool)), view,
          SLOT(scrollToBottom()));
  connect(actionScrollRight, SIGNAL(triggered(bool)), view,
          SLOT(scrollRight()));
  connect(actionScrollLeft, SIGNAL(triggered(bool)), view, SLOT(scrollLeft()));
  connect(actionScrollRightFast, SIGNAL(triggered(bool)), view,
          SLOT(scrollRightFast()));
  connect(actionScrollLeftFast, SIGNAL(triggered(bool)), view,
          SLOT(scrollLeftFast()));
  ...
  connect(actionScrollUpFast, SIGNAL(triggered(bool)), view,
          SLOT(scrollUpFast()));
  connect(actionScrollDownFast, SIGNAL(triggered(bool)), view,
          SLOT(scrollDownFast()));
  connect(actionRotateRight, SIGNAL(triggered(bool)), view,
          SLOT(rotateRight()));
  connect(actionRotateLeft, SIGNAL(triggered(bool)), view, SLOT(rotateLeft()));
  connect(actionNoRotation, SIGNAL(triggered(bool)), view,
          SLOT(resetRotation()));
  ...
  connect(view, SIGNAL(currentPageChanged(int)), this,
          SLOT(currentPageChanged(int)));
  connect(pageLoader, SIGNAL(pageLoaded(const Page &)), view,
          SLOT(setImage(const Page &)));
  connect(pageLoader, SIGNAL(pageLoaded(const Page &, const Page &)), view,
          SLOT(setImage(const Page &, const Page &)));
  connect(view, SIGNAL(pageReady(const Page &)), this,
          SLOT(pageLoaded(const Page &)));
  connect(view, SIGNAL(pageReady(const Page &, const Page &)), this,
          SLOT(pageLoaded(const Page &, const Page &)));
  connect(view, SIGNAL(requestPage(int)), pageLoader, SLOT(request(int)));
  connect(view, SIGNAL(requestTwoPages(int)), pageLoader,
          SLOT(requestTwoPages(int)));
  connect(view, SIGNAL(cancelPageRequest(int)), pageLoader, SLOT(cancel(int)));
  connect(view, SIGNAL(cancelTwoPagesRequest(int)), pageLoader,
          SLOT(cancelTwoPages(int)));
...
void ComicMainWindow::applyFullscreenSettings() {
  savedToolbarState =
      toolBar->toggleViewAction()->isChecked() || toolBar->isVisible();
...
void ComicMainWindow::enableComicBookActions(bool f) {
  ...
  actionPreviousFrame->setEnabled(f && actionFrameView->isChecked());
...
bool ComicMainWindow::confirmExit() {
  return QMessageBox::question(this, tr("Leave QComicBook?"),
                               tr("Do you really want to quit QComicBook?"),
                               QMessageBox::Yes,
...
void ComicMainWindow::sinkReady(const QString &path) {
  _DEBUG;
  statusbar->setShown(
      actionToggleStatusbar->isChecked() &&
      !(isFullScreen() &&
        cfg->fullScreenHideStatusbar())); // applies back user's
                                          // statusbar&toolbar preferences
  // toolbar->setShown(actiontoggleToolbar->isOn() && !(isFullScreen() &&
  // cfg->fullScreenHideToolbar()));
...
void ComicMainWindow::sinkError(int code) {
  ...
  switch (code) {
  case SINKERR_EMPTY:
    msg = tr("no images found");
    break;
  case SINKERR_UNKNOWNFILE:
    msg = tr("unknown archive");
    break;
  case SINKERR_ACCESS:
    msg = tr("can't access directory");
    break;
  case SINKERR_NOTFOUND:
    msg = tr("file/directory not found");
    break;
  case SINKERR_NOTSUPPORTED:
    msg = tr("archive not supported");
    break;
  case SINKERR_ARCHEXIT:
    msg = tr("archive extractor exited with error");
    break;
  default:
    break;
  }
  QMessageBox::critical(this, tr("QComicBook error"),
                        tr("Error opening comicbook") + ": " + msg,
...
void ComicMainWindow::browseDirectory() {
  const QString dir = QFileDialog::getExistingDirectory(
      this, tr("Choose a directory"), lastdir);
  if (!dir.isEmpty())
    open(dir, 0);
}
...
void ComicMainWindow::browseArchive() {
  const QString file = QFileDialog::getOpenFileName(
      this, tr("Choose a file"), lastdir,
      "Archives (" +
          ArchiversConfiguration::instance().supportedOpenExtensions().join(
              " ") +
          " *.pdf);;All files (*)");
  if (!file.isEmpty())
    open(file, 0);
...
void ComicMainWindow::open(const QString &path, int page) {
  ...
  connect(sink.data(), SIGNAL(progress(int, int)), statusbar,
          SLOT(setProgress(int, int)));

  statusbar->setShown(true); // ensures status bar is visible when opening
...
void ComicMainWindow::firstPage() { jumpToPage(0); }
...
void ComicMainWindow::forwardPages() { jumpToPage(currpage + 5); }

void ComicMainWindow::backwardPages() { jumpToPage(currpage - 5); }
...
void ComicMainWindow::currentPageChanged(int n) {
  ...
  currpage = n;
  const QString page = tr("Page") + " " + QString::number(n + 1) + "/" +
                       QString::number(sink->numOfImages());
  pageinfo->setText(page);
  statusbar->setPage(n + 1, sink->numOfImages());

  //
  // enable or disable next/prev/backward/forward page actions if first/last
  // page shown
...
void ComicMainWindow::showAbout() {
  AboutDialog *win = new AboutDialog(
      ...
      "<a "
      "href=\"https://github.com/stolowski/QComicBook\">https://github.com/"
      "stolowski/QComicBook</a><br>"
      "<a href=\"mailto:stolowski@gmail.com\">stolowski@gmail.com</a>",
      QPixmap(":/images/qcomicbook-splash.png"));
  win->show();
}
...
void ComicMainWindow::showConfigDialog() {
  ...
  connect(d, SIGNAL(displaySettingsChanged()), this,
          SLOT(reconfigureDisplay()));
  d->show();
}
...
void ComicMainWindow::showAboutQt() { QApplication::aboutQt(); }
...
void ComicMainWindow::removeBookmark() {
  _DEBUG;
  if (sink && bookmarks->exists(sink->getFullName()) &&
      QMessageBox::question(
          this, tr("Removing bookmark"),
...
void ComicMainWindow::savePageAs() {
  if (sink && (sink->numOfImages() > 0)) {
    ...
    for (int i = 0; i < cnt; i++) {
      ...
      if (cnt > 1) {
        tmpmsg +=
            " (" + tr("page") + " " + QString::number(currpage + i + 1) + ")";
      }
      QString fname = QFileDialog::getSaveFileName(this, tmpmsg, QString{},
                                                   "Images (*.jpg *.png)");
      if (fname.isEmpty()) {
        break;
      }
      ...
      if (result != 0 ||
          !img.getImage().save(
              fname)) // TODO: overwrite and default format (jpeg)
      {
        QMessageBox::critical(this, tr("QComicBook error"),
                              tr("Error saving image"), QMessageBox::Ok,
...
void ComicMainWindow::openPrintDialog() {
  ...
  if (printdlg.exec() == QDialog::Accepted) {
    ...
    printThread = new PrinterThread(sink, printer, printdlg.printRange(),
                                    printdlg.fromPage(), printdlg.toPage());
    connect(printThread, SIGNAL(printing(int)), progressdlg,
            SLOT(setPage(int)));
...
void ComicMainWindow::bookmarkSelected(QAction *action) {
  ...
  if (bookmarks->get(action, b)) {
    if (b.getName() != QString{}) {
      QString fname = b.getName();
      if (sink &&
          fname == sink->getFullName()) // is this comicbook already opened?
      {
