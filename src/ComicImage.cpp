...
ComicImage::ComicImage(PageViewBase *parent)
    : QGraphicsItem(), m_view(parent), m_sourceSize(0, 0), m_scaledSize(0, 0),
      m_pixmap(0) {}

ComicImage::~ComicImage() { delete m_pixmap; }
...
bool ComicImage::isDisposed() const { return m_pixmap == NULL; }
...
QSize ComicImage::getSourceSize() const { return m_sourceSize; }

QSize ComicImage::getScaledSize() const { return m_scaledSize; }

int ComicImage::width() const { return m_scaledSize.width(); }

int ComicImage::height() const { return m_scaledSize.height(); }
...
void ComicImage::recalcScaledSize() {
  ...
  int pixmapWidth,
      pixmapHeight; // resulting image size (with scaling and rotation applied)
  ...
  rmtx.scale(static_cast<double>(pixmapWidth) / totalWidth,
             static_cast<double>(pixmapHeight) / totalHeight);

  // setContentsMargins(xoff, yoff, 0, 0);
  // setFixedSize(m_scaledSize.width() + 2*xoff, m_scaledSize.height() +
  // 2*yoff);
...
void ComicImage::paint(QPainter *painter, const QStyleOptionGraphicsItem *opt,
                       QWidget *widget) {
  if (m_pixmap) {
    //      painter->drawPixmap(opt->rect().x(), opt->rect().y(), *m_pixmap,
    //      opt->rect().x()-xoff, opt->rect().y()-yoff, opt->rect().width(),
    //      opt->rect().height());
    painter->drawPixmap(opt->exposedRect, *m_pixmap, opt->exposedRect);
  }
}

void ComicImage::requestRedraw(const QSize &requestedSize,
...
void ComicImage::redraw(const QImage &img) {
  _DEBUG;
  if (img.size() == m_scaledSize) // sanity check; should match size requested
                                  // in requestRedraw(..)
  {
    if (m_pixmap != NULL &&
        m_pixmap->size() ==
            img.size()) // reuse existing pixmap if of right size
    {
      QPainter p(m_pixmap);
      p.drawImage(0, 0, img);
      p.end();
...
const QPixmap *ComicImage::pixmap() const { return m_pixmap; }

PageViewBase *ComicImage::view() const { return m_view; }
