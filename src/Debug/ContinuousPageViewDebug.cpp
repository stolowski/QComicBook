...
void ContinuousPageViewDebug::dumpTextData() {
  ...
  s << "Page description format: "
    << "d - disposed, e - estimated, pageNumber/estimatedSize; sourceSize; "
       "scaledSize / numOfPages"
    << "\n";
...
void ContinuousPageViewDebug::drawPageRectangles() {
  ...
  for (int i = 0; i < m_view->imgLabel.size(); i++) {
    ...
    m_scene->addRect(page->x(), page->y(), page->width(), page->height(),
                     QColor(Qt::blue));
    m_scene->addRect(page->x(), page->y(), page->width(), page->height(),
                     QColor(Qt::blue));
    ...
    status +=
        QString::number(es.width()) + "x" + QString::number(es.height()) + ";";
    ...
    status +=
        QString::number(ss.width()) + "x" + QString::number(ss.height()) + "/";
